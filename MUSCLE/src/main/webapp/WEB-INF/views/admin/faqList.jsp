...
<div align="center" style="width:1000px; margin-left:auto; margin-right:auto;">
   ...
   <div style="padding:10px; float:right; display:inline-block;">
      <form id="frm" method="post" action="/muscle/admin/SearchAcs.do">
        ...
        <input type="submit" value="검색" class="btn btn-primary btn-sm" style="display:inline-block; margin-bottom:5px; background-color:pink; border-color:pink;"/>
    </form>
   </div>
   ...
   <table align="center" class="table table-striped table-hover" style="cellpadding:7px;">
         ...
         <c:when test="${fn:length(list) > 0 }">
            <c:forEach items="${list}" var="row">
               <tr>
                  ...
                  <td><a href="#this" name="title" style=color:hotpink;>${row.NOTICE_TITLE}</a> <input type="hidden" id="NOTICE_NUM" value="${row.NOTICE_NUM}"></td>
      ...
      <button type="button" class="btn btn-primary btn-sm" style="background-color:pink; border-color:pink;" onclick="location.href='/muscle/admin/openAcsWrite.do'">글 작성</button>
      </c:if>
      </div>  
   </div>   
  </div>
